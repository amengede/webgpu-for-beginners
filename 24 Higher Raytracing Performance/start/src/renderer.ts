...
export class Renderer {
   ...
   async Initialize() {
    ...
    render = () => {
        ...
        let renderStart: number = performance.now();
        ...
        this.device.queue.onSubmittedWorkDone().then(
            () => {
                ...
                if (renderTimeLabel) {
                    renderTimeLabel.innerText = (end - renderStart).toFixed(2).toString();
                }
                let frameTimeLabel: HTMLElement =  <HTMLElement> document.getElementById("frame-time");
                if (frameTimeLabel) {
                    frameTimeLabel.innerText = (end - start).toFixed(2).toString();
                }
            }
        );
