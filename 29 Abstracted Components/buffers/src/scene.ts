...
export class Scene {
    ...
    triangleIndexData: Float32Array;
    nodes: Node[]
    nodesUsed: number = 0
    tlasNodesMax: number
    blasIndices: number[]
    blasIndexData: Float32Array;
    ...
    constructor() {
        ...
        for (let y = -2; y <= 2; y++) {
            for (let x = -2; x <= 2; x++) {
                for (let z = -2; z <= 2; z++) {
                    this.statues[i] = new Statue([2 * x, 2 * y, 4 * z], [180, 0, 90]);
                    i += 1;
                }
            }
        }
    ...
    async make_scene() {
        ...
        this.triangleIndexData = new Float32Array(this.triangles.length);
        ...
        this.flattenTriangles();
    }

    flattenTriangles() {
        for (let i = 0; i < this.triangles.length; i++) {
            this.triangleIndexData[i] = this.triangleIndices[i];
        }
    }

    flattenObjects() {
        for (let i = 0; i < this.blasIndices.length; i++) {
            this.blasIndexData[i] = this.blasIndices[i];
        }
    }
    ...
    buildBVH() {
        ...
        this.blasIndexData = new Float32Array(this.statues.length);
